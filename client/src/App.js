...
class App extends Component {
  ...
  state = { storageHashCAD: "", validate: null, showValidate: true, showRegister: false, numberOfRegistrations: 0, web3: null, accounts: null, contract: null, isDeploymentOwner: null, balance: null, addressContract: null };
  constructor(props) {
    super(props);
  ...
  handleSubmit = async () => {
    const { storageHashCAD, accounts, contract } = this.state;
    const registered = await contract.methods.findHash(storageHashCAD).call();
    console.log(storageHashCAD);
    // Se comprueba que los campos no esten vacios
    if (storageHashCAD === "") {
      Swal.fire({
        icon: 'warning',
        title: '¡Atención!',
        text: 'Los campos deben estar completos para registrar el comprobante digital en la red Blockchain',
    ...
    // Se comprueba que no este registrado
      ...
      try {
        let result = await contract.methods.registerCAD(storageHashCAD, accounts[0]).send({ from: accounts[0] });
        Swal.fire({
          icon: 'success',
          title: '¡Correcto!',
          text: 'El comprobante digital se registró en la red Blockchain con éxito',
          confirmButtonText: 'Aceptar',
        });
        this.resetForm();
        console.log(result);
        ...
        Swal.fire({
          icon: 'warning',
          title: '¡Atención!',
          html: 'Error al registrar el comprobante digital <br />' +
      ...
      Swal.fire({
        icon: 'warning',
        title: '¡Atención!',
        text: 'El comprobante digital ya se encuentra registrado en la red Blockchain',
        confirmButtonText: 'Aceptar',
      });
    }
  ...
  handleValidate = async () => {
    const { storageHashCAD, contract } = this.state;
    if (storageHashCAD === "") {
      Swal.fire({
        icon: 'warning',
        title: '¡Atención!',
        text: 'Los campos deben estar completos para validar la autenticidad del comprobante digital',
        confirmButtonText: 'Aceptar',
      });
    } else {
      try {
        const response = await contract.methods.validateCAD(storageHashCAD).call();
        ...
        Swal.fire({
          icon: 'warning',
          title: '¡Atención!',
          html: 'Error al validar el comprobante digital <br />' +
            'Inténtelo más tarde o revise el siguiente mensaje:' +
            '<hr style="border:1px dashed #dfdfdf; width:90%"></hr>' +
            '<span style="color:red">Error: </span>' +
            error.message,
        ...
        <div className="container row">
          ...
          <div className="col s12 m8">
            ...
            <div className="alert card amber lighten-4 brown-text">
              ...
              <div className="card-content">
                <p>
                  <i className="fas fa-exclamation-circle"></i> <span>Atención:</span>Estar conectado a la red <strong>Ethereum</strong>
                </p>
              </div>
            </div>
          </div>
    ...
    return (
      <div className="container">
        ...
        <div className="center">
          <h5 className="principal-title">Validación de autenticidad de comprobantes digitales</h5>
          <br />
        </div>
        <div className="row">
            ...
            <div className="row">
              <div className="col s12 m6">
                <ul className="tabs container-one">
                  <li className="tab col s6"><button onClick={this.handleChangeValidate}>Validar comprobantes</button></li>
                  <li className="tab col s6 active"><button className="active" onClick={this.handleChangeRegister}>Registrar comprobante</button></li>
                </ul>
              </div>
            </div>
          }
          {this.state.showValidate === true && this.state.isDeploymentOwner === true &&
            <div className="row">
              <div className="col s12 m6">
                <ul className="tabs container-one">
                  <li className="tab col s6 active"><button data-testid="btnChangeValidate" className="active" onClick={this.handleChangeValidate}>Validar comprobantes</button></li>
                  <li className="tab col s6"><button data-testid="btnChangeRegister" onClick={this.handleChangeRegister}>Registrar comprobante</button></li>
                </ul>
              </div>
            </div>
          }
                  ...
                  <div className="row">
                    <div className="file-field input-field">
                      ...
                      <div className="file-path-wrapper">
                        ...
                        <span className="helper-text" data-error="El comprobante seleccionado debe ser un archivo PDF" data-success="El comprobante seleccionado tiene el formato adecuado" >Subir el comprobante  digital en formato PDF</span>
                      </div>
                    </div>
                    {this.state.storageHashCAD !== "" &&
                      <div className="input-field">
                        <span className="helper-text"><i className="fas fa-info-circle"></i> SHA-256 del comprobante: {this.state.storageHashCAD}</span>
                      </div>
                    }
                  </div>
                  <div className="center">
                    <button type="button" data-test-id="test-button-register" className="btn btnBlueUNL" disabled={this.state.storageHashCAD === ""} onClick={this.handleSubmit}>Registrar en Blockchain</button>
                  </div>
                </form>
                <div className="alert card blue lighten-4 blue-text text-darken-3">
                  <div className="card-content">
                    <p><i className="fas fa-info-circle"></i> <span>Información:</span> Actualmente existen <span>{this.state.numberOfRegistrations}</span>comprobantes digitales registrados en la red de Ethereum.</p>
                  </div>
                </div>
              </div>
            }
            ...
            {this.state.showValidate === true &&
              <div className="col s12 m6">
                <form ref={(el) => this.myFormRef = el}>
                  ...
                  <div className="center">
                    <button type="button" data-test-id="test-button-validate" className="btn btnBlueUNL" disabled={this.state.storageHashCAD === ""} onClick={this.handleValidate}>Validar</button>
                  </div>
                </form>
              </div>
            }
                ...
                <div className="row " >
                  <div className="col s12">
                    <div className="card" ref={this.cardValidate}>
                      <div className="card-content">
                        <span className="">El comprobante digital:</span> <br /> <br />
